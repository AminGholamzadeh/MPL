...
#define log_i       debugSerial.printf
#define log_e       debugSerial.printf
 ...
 *  @return     0 if successful.
 */
int mpu_get_temperature(int32_t *data)
{
    unsigned char tmp[2];
    short raw;
...
static int setup_compass(void)
{
    ...
    for (akm_addr = 0x00; akm_addr <= 0xFE; akm_addr++) {
        int result;
        result = i2c_read(akm_addr, AKM_REG_WHOAMI, 1, data);
        if (data[0] == AKM_WHOAMI)
        log_e("addr = 0x%x, whoami = 0x%X \r\n", akm_addr, data[0]);
 ...
 *  @return     0 if successful.
 */
int mpu_get_compass_reg(int16_t *data)
{
#ifdef AK89xx_SECONDARY
    unsigned char tmp[9];
